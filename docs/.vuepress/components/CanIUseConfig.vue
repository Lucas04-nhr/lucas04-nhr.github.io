...
import {
  useCaniuse,
  useCaniuseFeaturesSearch,
  useCaniuseVersionSelect,
} from "../composables/caniuse.js";
...
const { feature, featureList, onSelect, isFocus } = useCaniuseFeaturesSearch(
  inputEl,
  listEl,
);
const { past, pastList, future, futureList, embedType, embedTypeList } =
  useCaniuseVersionSelect();
      ...
      <label class="caniuse-form-item" :for="`caniuse-feature-input-${id}`">
        ...
        <div class="feature-input">
          ...
          <ul v-show="isFocus" ref="listEl" class="feature-list">
            <li v-for="item in featureList" :key="item.value">
              <button
                type="button"
