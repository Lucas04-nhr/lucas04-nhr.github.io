...
permalink: /docs/r-course/talk03/
---
## R language basics, part 2 {#talk03}
...
The book updated to 2^nd^ ed. on July,2023, here’ s a [link](https://r4ds.hadley.nz) to the official website.
  ...
  <LinkCard 
    ...
    href="[/docs/hust-gp-template/faqs/](https://github.com/Lucas04-nhr/R-for-Data-Science/blob/main/talk03.pdf)" > 
  </LinkCard>

  <LinkCard 
    icon="ic:round-document-scanner"
    title="View the original Rmd file"
    href="https://github.com/Lucas04-nhr/R-for-Data-Science/blob/main/talk03.Rmd" > 
  </LinkCard>
  :::
...
### **What is a `data.frame`?** {#what-is-data-frame}
...
### **Usage of `head()` and `tail()`** {#usage-of-head-and-tail}
...
### Components of `data.frame` and common functions {#components-and-functions-of-data-frame}
...
### Structure of `data.frame` & `tibble` {#structure-of-data-frame-and-tibble}

`str(mpg)`

This command shows the structure of the tibble `mpg`:

![Structure of `mpg`](https://cdn.jsdelivr.net/gh/Lucas04-nhr/Pictures@main/uPic/Structure%2520of%2520mpg.png)

### Make a new `data.frame` {#make-new-data-frame}
...
### Manipulate the `data.frame` {#manipulate-data-frame}
...
### Bind several `data.frame` {#bind-several-data-frames}
...
### Make new `tibble` {#make-new-tibble}
...
### `add_row()` and `add_column()` {#add-row-and-add-column}

These 2 functions are too simple to explain.

### Manipulate the `tibble` {#manipulate-tibble}

::: note
See [Manipulate the `data.frame`](#manipulate-data-frame)
:::

### `tibble` to `data.frame` {#tibble-to-data-frame}

You can use the following functions to convert between `tibble` and `data.frame`:
...
## Differences between tibble and data.frame {#differences-between-tibble-and-data-frame}

### Tibble evaluates columns sequentially {#tibble-evaluates-columns-sequentially}
...
### `data.frame` causes trouble when fetching `subset` operations {#data-frame-causes-trouble-when-fetching-subset-operations}
...
###  `tibble` allows controlled data type conversion {#tibble-allows-controlled-data-type-conversion}

::: note
No proper example in the slide, following example is generated by AI.
:::

When reading data from files, `data.frame` and `tibble` handle character columns differently. By default, `data.frame` converts character columns to factors, while `tibble` keeps them as characters. This can lead to unexpected behavior if not managed properly.

```r ::collapsed-lines=10
# Create a small CSV file
cat("name,age\nAlice,30\nBob,25\n", file = "tmp_example.csv")

# Base R: read.csv with stringsAsFactors = TRUE (explicit)
df_base <- read.csv("tmp_example.csv", stringsAsFactors = TRUE)

# tibble via readr::read_csv keeps character columns as character
library(readr)
df_tibble <- read_csv("tmp_example.csv", show_col_types = FALSE)
...
sapply(df_tibble, class)
#> name  age 
#> "character" "integer"

# Controlled conversion with readr: parse 'name' as a factor with known levels
df_controlled <- read_csv(
  "tmp_example.csv",
  col_types = cols(
    name = col_factor(levels = c("Alice", "Bob")),
    age  = col_integer()
  ),
  show_col_types = FALSE
)

sapply(df_controlled, class)
#> name  age 
#> "factor" "integer"
```
...
`data.frame` can be recycled when the length of columns are not equal but divisible, while `tibble` can only recycle when the length is 1 or equal.
...
`data.frame` will do partial matching, while `tibble` will ==**NEVER**=={.caution} do it.
...
## Advanced tips for using `data.frame` and `tibble` {#advanced-tips-for-using-data-frame-and-tibble}
...
### Read from other formats {#read-from-other-formats}

To read from other formats, you can try the following packages:

::: note
Similar to Python, R has a rich ecosystem of packages for reading various file formats.
:::
